...
JitConstants GRUKernelBase::GetJitConstants(const gru_params& params) const {
    ...
    jit.AddConstants({
        ...
        MakeJitConstant("GEMM_OFFSET_R", params.GetOffsetIndexR() * size),
        MakeJitConstant("GEMM_OFFSET_H", params.GetOffsetIndexH() * size),
    });
...
KernelsData GRUKernelBase::GetCommonKernelsData(const Params& params) const {
    if (!Validate(params)) {
        return {};
    }

    const gru_params& orgParams = static_cast<const gru_params&>(params);
    bool sequential = orgParams.sequential;
    KernelData kd = KernelData::Default<gru_params>(params, 1);
