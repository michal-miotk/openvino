...
)
{
    ...
    unroll_for(uint i=0;i<real_seq_length;++i){
        ...
        unroll_for(uint l=0;l<NUM_HIDDEN_TO_DO;++l) { //kernel responsible for HIDDEN_SIZE
            ...
            unroll_for(uint k=0;k<GATE_NUM;++k){
                ...
                switch(k){
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        gate_output[k] = ACTIVATION_G(ACTIVATION_CLIP(TO_OUTPUT_TYPE(gate_output[k]*gate_output[0]), ACTIVATION_PARAMS_CLIP), ACTIVATION_PARAMS_G);
                        break;
                    default:
                        break;
                }
            ...
            #endif
            #ifdef SEQUENCE
                hidden_state[OUTPUT1_GET_INDEX_SAFE(b, 0, hidden_idx, 0)] = temp_state;
            #else
                hidden_state[OUTPUT_GET_INDEX_SAFE(b, hidden_idx, 0, 0)] = temp_state;
            #endif
            #ifdef SEQUENCE
