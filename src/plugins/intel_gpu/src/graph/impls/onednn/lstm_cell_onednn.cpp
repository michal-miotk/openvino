...
struct lstm_cell_onednn : typed_primitive_onednn_impl<lstm_cell> {
    using parent = typed_primitive_onednn_impl<lstm_cell>;
    using parent::parent;

    DECLARE_OBJECT_TYPE_SERIALIZATION(cldnn::onednn::lstm_cell_onednn)
...
protected:
    ...
    static std::shared_ptr<dnnl::lstm_forward::primitive_desc> get_lstm_primitive_descriptor(const kernel_impl_params& impl_params, cldnn::engine& engine,
        ...
        auto prim = impl_params.typed_desc<lstm_cell>();
        const auto& src_shape = impl_params.get_input_layout(0).get_shape();
        auto mod_src_shape = src_shape;
        mod_src_shape = {1, mod_src_shape[0], mod_src_shape[1], 1};
        ...
        initial_hidden_shape_mod = { 1, 1, initial_hidden_shape_mod[0], initial_hidden_shape_mod[1] };
        ...
        W_shape_mod = {1, 1, W_shape_mod[1], 4, W_shape_mod[0]/4};
        ...
        R_shape_mod = {1, 1, R_shape_mod[1], 4, R_shape_mod[0]/4};
        ...
        B_shape_mod = {1, 1, 4, B_shape_mod[0]/4};
        ...
        out_shape = {1, out_shape[0], out_shape[1], 1};
        ...
        OPENVINO_ASSERT(input_md.get_format_kind() != dnnl::memory::format_kind::any,
                        "[GPU] The format kind of the input memory descriptor of onednn lstm_cell cannot be 'any'.");
        OPENVINO_ASSERT(output_md.get_format_kind() != dnnl::memory::format_kind::any,
                        "[GPU] The format kind of the output memory descriptor of onednn lstm_cell cannot be 'any'.");
...
public:
    ...
    static std::unique_ptr<primitive_impl> create(const lstm_cell_node& arg, const kernel_impl_params& impl_params) {
        ...
        return cldnn::make_unique<lstm_cell_onednn>(engine, config, attr, *prim_desc, get_weights_reorder(impl_params, *prim_desc));
    }
};
...
std::unique_ptr<primitive_impl> LSTMCellImplementationManager::create_impl(const program_node& node, const kernel_impl_params& params) const  {
    ...
    return onednn::lstm_cell_onednn::create(static_cast<const lstm_cell_node&>(node), params);
}
