...
std::vector<layout> lstm_seq_inst::calc_output_layouts(lstm_seq_node const& node, kernel_impl_params const& impl_param) {
    ...
    auto input_layout_initial_cell_state = impl_param.get_input_layout(2);
    auto input_pshape_initial_cell_state = input_layout_initial_cell_state.get_partial_shape();
    int lstm_batch_size, lstm_seq_length, lstm_hidden_size;
    if (input_pshape_initial_cell_state[0].is_static()) {
        lstm_batch_size = input_pshape_initial_cell_state[0].get_length();
    } else {
        lstm_batch_size = -1;
    }
    ...
    if (input_pshape_initial_cell_state[2].is_static()) {
        lstm_hidden_size = input_pshape_initial_cell_state[2].get_length();
    } else {
        lstm_hidden_size = -1;
    }
