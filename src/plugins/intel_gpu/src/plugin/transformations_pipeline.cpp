    ...
    bool unroll_loop = config.get_property(ov::intel_gpu::enable_loop_unrolling);
    {
        ...
        auto isSequencePrimitiveSupported = [](const_node_ptr &node) -> bool {
            ...
            if (std::dynamic_pointer_cast<const ov::op::v5::RNNSequence>(node)) {
                ...
                return gru_seq->get_clip() == 0.0f && gru_seq->get_activations() == std::vector<std::string>{"sigmoid", "tanh"} &&
                       !ov::op::util::is_seq_len_provided(gru_seq->get_input_node_shared_ptr(0),
                                                          gru_seq->get_input_node_shared_ptr(2)) && gru_seq->get_linear_before_reset();
