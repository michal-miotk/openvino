...
static void CreateLSTMSequenceOp(ProgramBuilder& p, const std::shared_ptr<ov::op::v5::LSTMSequence>& op) {
    ...
    cldnn::lstm_seq prim({lstm_seq_id + ".out_pre_permute", permute_id_1, permute_id_2, \
    ...
    p.add_primitive(*op, prim);
    p.add_primitive(*op, cldnn::permute(lstm_seq_id + ".out0", lstm_seq_id + ".out_pre_permute", {1, 3, 0, 2}));
    p.add_primitive(*op, cldnn::mutable_data(lstm_seq_id + ".out1", {cldnn::input_info(lstm_seq_id + ".out_pre_permute")}, shared_memories.front()));
    p.add_primitive(*op, cldnn::mutable_data(lstm_seq_id + ".out2", {cldnn::input_info(lstm_seq_id + ".out_pre_permute")}, shared_memories.back()));
}
