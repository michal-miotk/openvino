...
// Radix test: f32 values whose sortable keys share the top 16 bits, so the K-th element can
// only be resolved by scanning all four radix digits.
TEST(arg_max_gpu_topk_radix, f32_max_low_mantissa_bits_only) {
    static const int32_t feature_num = 1024;
    auto& engine = get_test_engine();
    const int top_k = 64;
    auto input = engine.allocate_memory({data_types::f32, format::bfyx, {1, feature_num, 1, 1}});
    auto top_k_input = engine.allocate_memory({data_types::f32, format::bfyx, {1, 1, 1, 1}});
    auto second_output = engine.allocate_memory({data_types::f32, format::bfyx, {1, top_k, 1, 1}});

    // 1.0f + i ULP: all values land in [1, 2) and differ only in the low mantissa bits
    std::vector<float> input_vec(feature_num);
    for (int i = 0; i < feature_num; i++)
        input_vec[i] = 1.0f + static_cast<float>(i) / 8388608.0f;
    set_values(input, input_vec);
    ...
    cldnn::mem_lock<float> idx_ptr(second_output, get_test_stream());
    for (int k = 0; k < top_k; k++) {
        const int expected_idx = feature_num - 1 - k;
        ASSERT_EQ(static_cast<int>(idx_ptr[k]), expected_idx) << "index mismatch at k=" << k;
        ASSERT_EQ(val_ptr[k], input_vec[expected_idx]) << "value mismatch at k=" << k;
    }
}
...
// Radix test: f32 negatives only, MIN mode
TEST(arg_max_gpu_topk_radix, f32_min_negatives) {
    static const int32_t feature_num = 512;
    auto& engine = get_test_engine();
    const int top_k = 8;
    auto input = engine.allocate_memory({data_types::f32, format::bfyx, {1, feature_num, 1, 1}});
    ...
    std::vector<float> input_vec(feature_num);
    for (int i = 0; i < feature_num; i++)
        input_vec[i] = -1.0f - static_cast<float>((i * 37) % feature_num) * 0.5f;
    set_values(input, input_vec);
    ...
    std::vector<float> ref(input_vec);
    std::sort(ref.begin(), ref.end());

    cldnn::mem_lock<float, mem_lock_type::read> val_ptr(outputs.at("arg_max").get_memory(), get_test_stream());
    for (int k = 0; k < top_k; k++)
        ASSERT_EQ(val_ptr[k], ref[k]) << "value mismatch at k=" << k;
}

// Radix test: YOLOv26m-like scenario (N=24000, K=300, f32)
TEST(arg_max_gpu_topk_radix, f32_max_yolov26m_like) {
    static const int32_t feature_num = 24000;
    auto& engine = get_test_engine();
    const int top_k = 300;
    auto input = engine.allocate_memory({data_types::f32, format::bfyx, {1, feature_num, 1, 1}});
